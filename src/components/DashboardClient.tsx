...
function DashboardClient({ownerId}:{ownerId:string}) {
    ...
    const handleSettings=async ()=>{
        setloading(true)
        try {
            const result=await axios.post("/api/settings",{ownerId,buisnessName,supportEmail,knowledge})
    ...
    <div className='min-h-screen bg-zinc-50 text-zinc-900'>
      ...
      >
        <div className='max-w-7xl mx-auto px-6 h-16 flex items-center justify-between'>
          ...
          border-zinc-300 text-sm hover:bg-zinc-100
          transition'onClick={()=>navigate.push("/embed")}>Embed Chatbot</button>
        </div>
      </motion.div>
      <div className='flex justify-center px-4 py-14 mt-20'>
        ...
        >
            ...
            <div className='space-y-4'>
                ...
                focus:outline-none focus:ring-2 focus:ring-black/80' placeholder='Business Name' onChange={(e)=>setBuisnessName(e.target.value)} value={buisnessName ?? ""}/>
                <input type="text" className='w-full rounded-xl border border-zinc-300 px-4 py-3 text-sm
                focus:outline-none focus:ring-2 focus:ring-black/80' placeholder='Support Email'onChange={(e)=>setSupportEmail(e.target.value)} value={supportEmail}/>
            </div>
        </div>
         <div className='mb-10'>
